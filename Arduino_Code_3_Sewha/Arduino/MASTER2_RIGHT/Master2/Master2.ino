//Tree of Voices V2 --> https://github.com/moonryul/TreeOfVoiceForACC-master/tree/master/Arduino_Code/Master_Arduino_3 <-- using this code as reference
...
const int ss1 = 53; // connect master pin 53 the first slave pin 53 = pin 10 on Arduino UNO
...
const int ss5 = 46; // connect master pin 46 to the fifth slave pin 53

// A total num of LED = 186; each slave processes 40 LEDs
const int NumPixels1 = 27;
const int NumPixels2 = 30;
const int NumPixels3 = 27;
const int NumPixels4 = 25;
const int NumPixels5 = 25;

//const int m_totalNumOfPixels = NumPixels1 +  NumPixels2 + NumPixels3 +  NumPixels4 + NumPixels5 +2;
const int m_totalNumOfPixels = NumPixels1 +  NumPixels2 + NumPixels3 +  NumPixels4 + NumPixels5;
...
const int group5ByteSize = NumPixels5 * 3;
...
//SPISettings SPISettingE (4000000, MSBFIRST, SPI_MODE0); // 14MHz = speed; slave 5
...
void  sendLEDBytesToSlaves( byte * totalReceiveBuffer, int totalByteSize )
{
  ...
  digitalWrite(ss5, LOW); // select the fifth SS line

  SPI.transfer( m_startByte);
  SPI.transfer( &totalReceiveBuffer[group1ByteSize + group2ByteSize  + group3ByteSize + group4ByteSize], group5ByteSize );
